...
"""
╔══════════════════════════════════════════╗
║   🎯 BAGS.FM SCANNER BOT v3.0           ║
║   @underdeathsol | Twitter: @lilfid12   ║
╚══════════════════════════════════════════╝
  Fix: unique tokens, mini chart, multi-source API
"""
...
async def fetch_trending_tokens(session: aiohttp.ClientSession) -> list:
    ...
    # Bags.fm
    for url in [f"{BAGS_API_BASE}/v1/tokens/trending", f"{BAGS_API_BASE}/tokens/trending"]:
...
async def fetch_gainers(session: aiohttp.ClientSession) -> list:
    # Bags.fm
    for url in [f"{BAGS_API_BASE}/v1/tokens/gainers", f"{BAGS_API_BASE}/tokens/gainers"]:
...
async def fetch_token_detail(session: aiohttp.ClientSession, address: str) -> dict | None:
    # Bags.fm
    for url in [f"{BAGS_API_BASE}/v1/tokens/{address}", f"{BAGS_API_BASE}/tokens/{address}"]:
...
# ══════════════════════════════════════════════════════════════════════════════
def _buy_url(addr):
    return f"https://bags.fm/t/{addr}?ref=LILFID12"
...
def build_new_token_msg(token: dict, chart: str = "") -> tuple:
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💸  BUY on Bags.fm  🛒", url=_buy_url(addr))],
        [
...
def build_pump_msg(token: dict, pct: float, chart: str = "") -> tuple:
    ...
    kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("💸 BUY NOW 🚀", url=_buy_url(addr))],
        [
            InlineKeyboardButton("📊 Chart", url=f"https://bags.fm/t/{addr}"),
            InlineKeyboardButton("🦅 DexScreener", url=f"https://dexscreener.com/solana/{addr}"),
        ],
...
def build_leaderboard_msg(tokens: list) -> str:
    ...
    for i, t in enumerate(tokens[:10]):
        ...
        buy   = f"[💸](https://bags.fm/t/{addr}?ref=LILFID12)"
        lines.append(
            f"{medals[i]} *{name}* `${sym}`\n"
